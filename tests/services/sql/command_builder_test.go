...
func TestCommandBuilderResolvesAliasAsFirstPathSegment(t *testing.T) {
	...
	builder := sql.NewCommandBuilderWithAliases(aliases)
...
func TestDefaultCommandBuilderHasNoAliases(t *testing.T) {
	builder := sql.NewCommandBuilder()

	extractor := sql.NewExtractor()
	batchParser := sql.NewBatchParser(extractor)
	parser := sql.NewParser(extractor, batchParser, builder)

	command, err := parser.Parse("UPDATE md SET content=\"new\" WHERE content = \"old\"")
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if command.File != "md" {
		t.Errorf("expected literal file md, got %s", command.File)
	}

	if command.Action != models.UPDATE {
		t.Errorf("expected UPDATE action, got %v", command.Action)
	}
}
